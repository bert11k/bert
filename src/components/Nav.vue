<template>
  <nav :class="{'show': showSidebar}" class="container">
    ...
    <div class="nav-icons">
      <router-link :class="{'active': active === 'profile'}" class="item" to="/">
        <img alt="user" src="../assets/userwhite.png">
      </router-link>
      <router-link :class="{'active': active === 'projects'}" class="item" to="/projects">
        <img alt="projects" src="../assets/projects.png">
      </router-link>
    </div>
    <div class="nav-links">
      <transition-group name="fade">
        <div :class="{'active': active === 'profile'}" key="1" v-show="showLink">
          Профиль
        </div>
        <div :class="{'active': active === 'projects'}" key="2" v-show="showLink">
          Рабочая область
        </div>
      </transition-group>
    </div>
